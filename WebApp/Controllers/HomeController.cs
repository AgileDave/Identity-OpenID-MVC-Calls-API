...
namespace WebApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            //get user name from current principal -- can also use 'preferred_username' claim
            // the claim grabbed here is an optional claim added to the token from the AAD App Registration
            var username = ClaimsPrincipal.Current.FindFirst("http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress").Value;
            ViewBag.Name = username;

            
            //acquire token to call web api
            var app = ConfidentialClientApplicationBuilder.Create(AuthenticationConfig.ClientId)
                  .WithClientSecret(AuthenticationConfig.ClientSecret)
                  .WithAuthority(new Uri(AuthenticationConfig.Authority))
                  .Build();
            string[] scopes = new string[] { AuthenticationConfig.WebApiTodoListScope };
            var token = await app.AcquireTokenForClient(scopes)
                    .ExecuteAsync();

            //make http call
            var client = new HttpClient();
            var url = $"{AuthenticationConfig.WebApiTodoListBaseAddress}/api/todolist";
            ...
            defaultRequestHeaders.Add("X-UserName", username);

            HttpResponseMessage response = await client.GetAsync(url);
            if (response.IsSuccessStatusCode)
            {
                ViewBag.Todos = await response.Content.ReadAsStringAsync();
            }
            else
            {
                ViewBag.Todos = await response.Content.ReadAsStringAsync();
